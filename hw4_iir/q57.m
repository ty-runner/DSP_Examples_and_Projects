wp = 0.2*pi; %0.2 pi radians
Ap = 1; %dB
ws = 0.4*pi; %0.4 pi radians
As = 50; %dB

T = 1;         % sampling period
%Part A: use bilinear transformation and the Chebyshev I approx approach to
%obtain a system function H(z) in the parallel form that satisfies the
%specs
Wp = (2/T)*tan(wp/2);
Ws = (2/T)*tan(ws/2);
[N, Omegac] = cheb1ord(Wp, Ws, Ap, As, 's');
disp(N)
disp(Omegac)
